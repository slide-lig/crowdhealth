...
//		map.on('moveend', this._zoomFunction, this);
	},
...
//		map.off('moveend', this._zoomFunction, this);
	},
...
L.PanoramioLayer = L.PanoramioLayer.extend({
	...
	options: {
		...
		setIcon: function (record, options) {
			...
			var icon = new L.DivIcon({
				...
				//html: '<img class="photo" onload="this.style.opacity=1" title="' + title + '" src="marker_icon/pinkFlagCurl.png"/>'
			});
            if (type === 1){
                return new L.Icon({iconUrl: 'marker_icon/pinkFlagCurl.png',iconSize:[30, 30],iconAnchor:[15, 15],popupAnchor:  [-10, -50]});
            }
            else if (type === 0 ){
                return new L.Icon({iconUrl: 'marker_icon/greenFlag.png',iconSize:[30, 30],iconAnchor:[15, 15],popupAnchor:  [-10, -50]});
            }
			//return icon;
		},
	...
	onAdd: function (map) {
		...
		var resetFunction = function (e) {
			...
			me._call = setTimeout(request, 200);
		};
	...
	onRemove: function (map) {
		...
		map.off('moveend', this._resetFunction);
	},
