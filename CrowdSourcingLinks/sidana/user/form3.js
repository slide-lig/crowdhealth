...
function getNewTweets(dataSetName) {
    var serviceUrl = "http://ec2-54-88-48-230.compute-1.amazonaws.com/sidana/user/getNewTweets.php?dataSetName=" + dataSetName;
    $.ajax({
        url:serviceUrl,
        async:true,
        dataType: "json",
...
function fillUpFormWithNewData(newTweets) {
    for(var i=1 ; i<=newTweets.length;i++){
        ...
        var option1 = getRadioElement(optionName,"related");
        var option2 = getRadioElement(optionName,"unrelated");
        var option3 = getRadioElement(optionName,"ambiguous");
        var option4 = getRadioElement(optionName,"notEnglish");

        var pElement = getPElement(uniqueTweetId,dataSetName,tweetId,tweetText);
        pElement.append(getBRElement());
        pElement.append(option1);
        pElement.append(getLabel("related"));
        pElement.append(getBRElement());
...
function postData(label,uniqueTweetId,tweetId,labelId) {
    var serviceUrl = "http://ec2-54-88-48-230.compute-1.amazonaws.com/sidana/user/saveTweetLabels.php";
    $.ajax({
        url:serviceUrl,
        type: "POST",
        data : {
